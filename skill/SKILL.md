...
## 检索：用户问「我之前收藏过 XX 吗」（把库里的东西拿出来用）

前面 8 步都是**存进去**，这一节是**拿出来用**。

**一次不中就换问法，别急着说「库里没有」。** 实测用户式的关键词组合很容易被
库里的高频词带偏：`Agent 记忆` 目标文章只排到第 14，而**去掉修饰词**只搜
`记忆`（或写全 `个人 AI 记忆系统`）就是第 1 名 —— 因为本库 15% 的段含
"Agent"，它把真正有区分度的"记忆"淹掉了。

建议顺序：

1. 先按用户原话搜；
2. 不理想就**去掉修饰词、只留核心概念**再搜一次（实测往往比加词有效）；
3. 还不够就换说法（同义词、换个视角）；
4. 三次都不中，才说「库里确实没有」。

```bash
curl -s "http://127.0.0.1:8765/api/search?q=<关键词>&limit=10"
```

参数：`mode`（`hybrid` 默认 / `dense` / `sparse`）、`category`、`tag`、
`date_from` / `date_to`、`limit`。

**读分数要看对字段**：返回里的 `rerank_score`（0–1）是**绝对相关度**；
`score` 只是 RRF 排名融合分，**只反映「两路排在第几」，不反映相关度** ——
别拿它判断「像不像」（实测库里没有答案的问题也能拿到 0.03）。

要读全文用 `GET /api/articles/{id}`。**已核查过的结论优先信**：卡片里的
`boundary` / `verify` 是**去外部取证**得到的（仓库实况、roadmap 打勾情况），
比文章正文更可信 —— 文章会加戏，核查结论不会。
