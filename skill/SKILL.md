...
**图片合辑（`kind=wx_album`）**：正文主体是图片、文字只有文案，**正文本身几乎没有可检索内容**——
但这恰恰是第 2.5 步 OCR 存在的意义：合辑的图通常文字密集（长图、表格、截图），
OCR 会把它们全部变成正文。实测全库已由此产出 **808 段 / 约 40 万字**图内文字。

**所以照常按 RAG 分类入库，不要因为「它是图集」就归到「直接全文」分类** ——
那会让已经 OCR 出来的文字进了库却不建索引，**等于白 OCR**。

> 2026-09-23 修正：本节此前写「语义检索基本搜不到它……更多是存档而非可检索……
> 可建议用户归到一个「直接全文」分类」。**那是 OCR 上线之前的结论，已经过时。**
...
8. **抓取走脚本，不自己写抓取代码。**
   脚本有问题时（抓不到、判定错、认不出某种页面）—— **报告给用户，由用户决定改不改**。
   **不要自己动手改脚本**，也不要在对话里临时拼一段抓取代码顶上。
   **为什么**（2026-09-22 踩过）：SKILL.md 与 `wx_fetch.py` 各有**三份拷贝** —— 运行时
   （`~/.workbuddy/skills/MyRag/`）、项目（`MyRag/skill/`）、开源副本（`MyRag-oss/skill/`）。
   改一份**不会同步另外两份**，而运行时那份**改完立刻生效**，等于绕过评审直接上生产；
   更坏的情况是反向的：改了项目源却没同步到运行时，**你以为改好了、线上行为根本没变**。
   而且漏同步带来的不一致，会在下一次同步时**把修复覆盖回去，让 bug 复活**。
   > 这条以前的写法是「脚本坏了就改脚本」—— 那句话被读成了动手授权，于是真的有人直接改了。
   > **授权必须明确来自用户，不能靠文档里的措辞推断。**
