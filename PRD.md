...
| 实现方 | CodeBuddy |
| 使用者 | 单人（JL），本机使用 |
| v2.1 变更 | **把「使用者的判断标准」变成一等配置，并把评测集做扎实**：① **新增 `categories.principles`（收录原则）** —— 见 §5.5.1。分类设置页新增一个输入框，写「我在意什么、什么算值得试」；注入 Skill 后在**第 4 步写摘要、第 6 步核查定 `verdict`** 时以它为准。这是「入库时注入个人意志」的落点：与「先存起来、等用时再让模型理解」相比，差别不在模型强弱，而在**能看到的材料**（入库时能去外部取证，使用时只能看到文章本身）；② **检索返回 `rerank_score`** —— 0–1 的绝对相关度，与 RRF `score` 是两回事。实测 RRF 分**无法**区分「库里有没有答案」（有答案最低 0.0152 < 无答案最高 0.0275，区间重叠）；③ **评测集 10 → 37 条**（v3）：补多篇相关、**无答案探针**、主题簇、极短 query、跨语言，以及一组**同领域有/无答案的对照题**（q24 问 ASR 无答案 vs q37 问 OCR 有答案）；④ `scripts/eval.py` 新增 `--fail-on-regression` **回归门**——指标比基线退化即以非零码退出；⑤ 明确**不做 RAGAS**：其四个核心指标里，两个需要「LLM 生成的答案」而本项目不生成答案，另两个已被 IR 指标覆盖（§8.5） |
...
**定位的修正（2026-09-18，探讨中）**：

使用者提出：**这个系统搜索信息的方式，是他过去做咨询时快速找信息方式的缩影** ——
在海量信息中**速览、积累、发散**。因此它的定位不是「个人知识库」，而是
**「项目上信息收集阶段的容器」**。

| | 个人知识库 | **项目信息收集容器** |
|---|---|---|
| 目标 | 长期沉淀，没有终点 | **有课题、有阶段、有交付** |
| 信息取舍 | 按「有没有用」 | **按「对这个课题有没有用」** |
| 结束 | 永不结束 | **阶段结束要输出** |

**基准换掉之后，几个既有决策反而更站得住**：

- 为什么不用 ima：它不按课题聚合，也不面向交付
- 为什么「入库时判断」值钱：判断标准应服从项目目标，不是通用价值判断
- 为什么卡片比检索值钱：**速览是第一个动作**，检索只是它的补充

**同时暴露了最大的缺口**：三个动作里，**「发散」基本是断的**（有实测证据）。
详细分析与候选迭代路径见 §17（探讨中，未定稿）。
...
| `principles` | **收录原则** —— 使用者本人的判断标准，注入 Skill，决定价值判断、`verdict`、核查侧重（§5.5.1） |
...
`criteria` 与 `principles` 的分工是本设计的一个要点：

| 字段 | 回答的问题 | Agent 何时用 |
|---|---|---|
| `criteria` | **这篇归哪类** | 第 3 步判类 |
| `principles` | **这篇值不值得、该怎么看** | 第 4 步写摘要 + 第 6 步核查定 `verdict` |

**为什么不做成全局的一条配置**：三类内容的判断标准本就不同 —— 开源项目看"能否自托管、
有没有正式 release"，观点文章看"判断有没有论据、作者立场是什么"。一套标准覆盖三类必然含糊。

**为什么不做成结构化的多个字段**（如拆成 intake / verdict / boundary 三条）：那是实现视角。
使用者的思考方式是"我在意什么"，写成一整段自然语言，LLM 反而更能用上其中的逻辑
（「若…则…」）；结构化字段会逼他把话说碎，填的时候还要先判断"这条该填哪个框"。

**这是「入库时注入个人意志」的落点**：配置在入库时就已确定，Agent 照它执行。
与「先存起来、等用时再由模型理解」的路线相比，差别不在模型强弱，而在**能看到的材料** ——
入库时加工可以去外部取证（拉 GitHub 实况、核对 roadmap 打勾情况），
使用时加工只能看到文章本身，而文章本身可能是夸大的。

**空值行为**：某类没写 `principles` 时，Skill 按通用口径（成熟度 / 维护活跃度 / 能否自托管）
判断，并**要求在 `boundary` 里写明用的是哪套口径** —— 不允许 Agent 自己编标准。
...
**背景（实测，2026-09-17 复核）**：**922 张配图 / 318 MB**，占全库体积 **96.5%**
（数据库 10.2 MB、原文 1.0 MB）。规模分布：<10KB **65 张**（疑似图标/表情）、
10–100KB 142 张、**100KB–1MB 688 张（主体）**、>1MB 26 张。
**体积最大的三张全是 GIF**（5.5 / 4.2 / 4.1 MB）—— 佐证下面的 L1 硬规则打中了要害。
其中大量是**无信息载体**：小图标、广告图、"点击关注"引导图、分割线、动图、人物照片。
这类图**入库即永久占空间**，事后清理还得另做一层遗忘逻辑。

**结论：放到入库环节过滤**（源头治理），而不是等它进来再清。

**为什么不做「人工逐张清理」**（2026-09-17 讨论后决定暂缓）：
面对 922 张图，人工清理有两个绕不开的问题 ——
① **逐张判断不现实**：得先有缩略图才看得过来（本机 PIL 未装），而且"这张有没有用"
本就该由规则先筛一遍，人工只该处理规则吃不掉的边角；
② **删图会破正文**：正文里是 `![](../../media/<id>/xx.png)`，删掉文件后点开文章
就是破图，必须同时改写正文引用（替换成「图片已清理」之类）。
既然 L1 硬规则（尺寸 / 比例 / GIF）能零成本干掉大部分无信息载体，
就从源头做，**不做事后清理工具**。真到需要时（规则跑完仍有大量残留）再回头考虑。
...
1. **OCR 文字写入正文** —— 顺带解决「图片内容搜不到」
   ...
   **注入的块引用独立成段**（见 §7.2 与 §8.4）—— 否则它会和正文合并成千字长段，
   而精排在检索时只读段前 320 字符，图内文字等于没参与排序（实测踩过）。

2. **误杀可接受**（使用者 2026-09-17 明确）—— 每篇文章都保留原始 `url`，
   图误删后**点链接即可回原文查看**。因此判据可以激进，不必为"可能的架构图"
   保留大量待定项。

**实现状态（2026-09-17）**：`scripts/ocr_images.py` 已实现，走系统 Vision，
用法 `python scripts/ocr_images.py <文章id>` / `--all` / `--dry-run`。**幂等**（重复跑不叠加），
在图片引用行**紧后面**插入文字 blockquote，**不动原引用** —— 删不删图是另一个决定。

**首次实测（T-20260907-01「Datawhale FDE 100案例集」，245 页 PDF 扫描件）**：

| 指标 | 值 |
|---|---|
| 规模 | **241 张图**，全部识别出有效文字（0 张被判无信息） |
| 提字量 | **140,982 字** |
| 耗时 | **83.5 秒**（约 0.35 秒/张） |
| 正文 | 9,688 → **427,206 字节** |
| 切段 | **1 段 → 173 段 / 130,293 token** |

**效果**：这篇此前「正文 0 字、完全检索不到」，OCR 后搜它**只存在于图片里的句子**
（如「真实企业AI落地案例集」）能直接命中。**至此才谈得上删图**：
文字已落地，删图是「弃载体」，不是「删内容」。

**与入库时过滤的关系**：入库时的 L1/L2 是**增量预防**（新文章不再存无信息图），
本脚本是**存量清理**（把已经进库的图变成文字）。两者互补，不重复。
...
**为什么块引用要独立成段（2026-09-19 实测后补）**：OCR 注入的图内文字正是块引用（`> 图片文字：…`，见 §6.4）。若它只是一行普通文本，就会和前后正文并进同一个段、再按标题合并成上千字的长段 —— 而**精排在检索时只读段落前 320 字符**（`RERANK_MAX_CHARS`，见 §8.4），于是图内文字**根本没参与排序**。

实测：206 个含图内文字的段，平均 **1265 字**、**99.5%** 超过 320 字符，`图片文字` 标记平均出现在**第 261 字符**处 —— 精排的前 320 字符刚好只够看到标记行本身，一个字的内容都读不到。

独立成段后，图内文字回到段首、可被精排看全，也能被**单独命中**：实测搜 `FDE CASEBOOK 100` 直接命中图集文章 `T-20260907-01` 第 1 位（精排 0.9855）。单篇图集文章 `T-20260907-01` 由 **174 段 / 平均 1231 字** 变为 **485 段 / 平均 476 字**，超 320 字符的比例 **100% → 48%**。

**块引用同时不带重叠前缀** —— 153 字符的前缀会挤掉精排窗口里真正的内容，而它的上文已由相邻段完整保留。
...
**记录时间**：2026-09-18。使用者提出「项目信息收集容器」这个新定位（§1 末尾）后，
迭代顺序需要重新审视。**本节是探讨记录，不是承诺。**

### 17.1 三个动作的现状（咨询式信息收集）

| 动作 | 咨询里怎么做 | 本系统现状 |
|---|---|---|
| **速览** | 海量信息里快速筛出值得细看的 | ✓ **最强的一环**：卡片给结论 + 能力边界 + 核查证据 |
| **积累** | 存下来、可回溯 | ✓ 入库 + 配图过滤与 OCR（§6.4） |
| **发散** | 从一个线索滚出一片 | ✗ **基本断了**，见 17.2 |
...
```
概念数 280  |  出现次数 280      ← 平均每个概念只出现在 1 篇文章里
概念样例：10-20-70法则 / 5 数字 3 比率 / 6 步工作流 / 80/15/5 选型法则 …
```

每个概念都**只属于一篇**，所以「点概念看相关文章」这个交互**实际不存在**。

**根因是设计问题而非配置问题**：概念从**单篇**内抽取（`concepts_max: 7`），
**从未跨文章对齐** —— 它本质上是「术语表」，不是「共享概念」。

`tags` 同理：实测样例是 `["记忆系统","RAG","向量检索"]`，描述的是「这篇讲了什么」，
而不是「这篇属于哪个课题」。

**证据二：没有「课题」这个维度**

现有 `categories` 是**内容类型**（开源项目 / 技术科普 / 理念思考），回答「这篇是什么」；
而项目容器需要的是**课题归属**，回答「这篇属于哪个项目」。两者**正交**：
一篇只能属一个内容类型，但可以同时属于多个课题。

### 17.3 候选路径（优先级待确认）

```
① 概念归一 + 反查     ← 让「发散」真的能用（盘活已有的 280 个概念）
② 项目 / 课题维度     ← 与分类正交的新维度
③ 阶段性输出          ← 项目收尾要交付，而不是永久沉淀
```

**建议先做 ①**，三条理由：**不用新增数据**（概念已抽好，缺的只是归一与反查）；
**做完立刻可用**（概念 → 它出现在哪几篇 → 顺着滚下去）；
**它是 ②③ 的地基**（课题由概念连起来，概念连不上，课题就只是一堆散的）。

### 17.4 待使用者确定（阻塞 ②③）

1. **「项目」是什么形态**？一组文章的集合（像文件夹）／可打的标签（一篇可属多个）／
   还是「某个课题的看板」？
2. **现在有真实项目吗**，还是对以后用法的设想？

**若暂无具体项目**：建议**只做 ①**。等项目真的来了再设计 ②，
避免照想象搭一套用不上的结构 —— 这与 §6.4 里「不做人工清理工具」是同一种克制。
